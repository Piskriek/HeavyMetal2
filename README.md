...
Road, terrain, rock, timber, iron, lava, and water materials use the deliberately restrained repeat set generated by `scripts/generate-quiet-textures.mjs`: broad hand-painted value masses, softened seams, a narrow luminance range, and no grain or speckle. `public/art/deck-surface.png` remains a material for timber machinery, not the road. The rear-loading launcher and the ramps/loops retain their cached timber/iron appearance, and atlas reference scales prevent double scaling with the wider camera.
