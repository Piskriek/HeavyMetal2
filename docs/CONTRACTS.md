...
## T02 adoption note (commands gate fix)

The original `validateCommand` case for `steer` only allowed `ready` while its comment
and error message promised "the grid or while racing". T02 adopted the gate for the
headless seam and corrected the condition to `ready | flying | paused`; input-disabled
gating is unchanged. The contract tests now pin steering in all three states. No shape,
field or frozen decision changed, so `CONTRACTS_VERSION` stays 1.
