# Art wave 2: the rest of the game's missing art (one PR, four agents at once)

Wave 2 lands on the branch **`art/generated-wave-2`**, which starts from `art/generated-wave-1` (so the
wave-1 manifest is already there). Open the PR with base `art/generated-wave-1`. Four Codex agents work
...
|---|---|---|
| `src/game/meta/painted-parts.generated.ts` | W2-A1, W2-A2 | Never hand-edit. On a rebase conflict, take either side, then run `node --import tsx scripts/key-art.ts --set avatar-parts` (no `--only`). |
