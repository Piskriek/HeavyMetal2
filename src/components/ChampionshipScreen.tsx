...
function MarbleDot({ marble }: { marble: MarbleInfo }) {
  return <i className="tiny-marble" style={{ '--marble': marble.color } as CSSProperties} />;
}
...
export default function ChampionshipScreen({ season, onStartHeat, onRetune, onAbandon, onNewSeason, account, onShop }: Props) {
  ...
  return <div className="app-shell championship-page">
    <header className="app-header"><Brand onClick={onAbandon} /><nav className="main-nav" aria-label="Main navigation"><button onClick={onAbandon}>Garage</button><button className="active" aria-current="page">Championship</button><button onClick={() => setDialog('rules')}>How to play</button></nav><div className="header-tools"><button className="text-button lab-link" onClick={() => setDialog('lab')}><FlaskConical size={16} /><span>Physics lab</span></button><WalletButton credits={account.credits} onClick={onShop} /></div></header>
    <main className="garage-main">
      <div className="garage-heading entrance"><div><div className="eyebrow"><span className="accent-line" /> WORLD MARBLE CHAMPIONSHIP <span className="muted">/ {season.seed.toString(16).slice(0, 4).toUpperCase()}</span></div><h1>CHASE THE <span>CHAMPIONSHIP.</span></h1><p>Six circuits. Eighteen heats. Every position matters.</p></div><button className="button-secondary" onClick={onAbandon}>Garage <ArrowUpRight size={17} /></button></div>

      {champion && <section className="champion-banner entrance"><Trophy size={45} /><div><span className="eyebrow">WORLD CHAMPION</span><h2>{champion.isPlayer ? 'YOU DID IT.' : `${champion.name.toUpperCase()} TAKES THE TITLE.`}</h2><p>{standings[0].points} points. {standings[0].wins} Grand Prix wins. {teams[0].team.name} wins the constructors' championship.</p><span>You finished P{playerPosition} in the championship.</span></div><button className="button-primary" onClick={onNewSeason}>New championship <ArrowRight size={18} /></button></section>}

      <LoadoutPreview inventory={account.inventory} onShop={onShop} />
      <div className="champ-layout entrance entrance-delay">
        <div className="champ-main-column">
          {!season.complete && <section className="next-event" aria-labelledby="next-event-title"><div className="section-topline"><span className="eyebrow"><Flag size={14} /> ROUND {String(round + 1).padStart(2, '0')} OF 06</span><span className="local-save"><Check size={12} />PROGRESS SAVED LOCALLY</span></div><div className="next-gp-title"><h2 id="next-event-title">{gp.name.toUpperCase()}</h2><p>{gp.desc.replace(/—/g, '-')}</p></div><div className="heat-progress" aria-label={`${completedHeats} of 3 heats complete`}>{Array.from({ length: 3 }, (_, i) => <div key={i} className={i < completedHeats ? 'heat-complete' : i === completedHeats ? 'heat-current' : ''}><span>{i < completedHeats ? <Check size={12} /> : String(i + 1).padStart(2, '0')}</span><b>HEAT {i + 1}</b><small>{i < completedHeats ? 'COMPLETE' : i === completedHeats ? 'UP NEXT' : 'SAME CIRCUIT'}</small></div>)}</div><CircuitPreview seed={gpSeed(season.seed, round)} roster={season.roster} profile={gp.profile} /><div className="next-gp-actions"><div><span className="eyebrow">APEX RACING / YOUR SETUP</span><p>W <b>{player.stats.weight}</b><span>/</span>S <b>{player.stats.speed}</b><span>/</span>B <b>{player.stats.bounce}</b></p></div><button className="button-secondary" onClick={onRetune} disabled={setupLocked} title={setupLocked ? 'Setup is locked until the next Grand Prix' : 'Tune your marble for this circuit'}>{setupLocked ? <LockKeyhole size={15} /> : <SlidersHorizontal size={15} />}{setupLocked ? 'Setup locked' : 'Retune'}</button><button className="button-primary" onClick={onStartHeat}>START HEAT {completedHeats + 1}<ArrowRight size={18} /></button></div><p className="parc-ferme-note">{setupLocked ? 'Parc ferme: your setup stays locked for all three heats.' : 'Fine-tune now. Your setup locks when the first heat starts.'}</p></section>}
        ...
        <aside className="standings-column"><div className="section-topline"><h2>THE STANDINGS</h2><span className="eyebrow">LIVE POINTS</span></div><div className="standings-tabs" aria-label="Championship standings"><button className={tab === 'drivers' ? 'selected' : ''} aria-pressed={tab === 'drivers'} onClick={() => setTab('drivers')}>Drivers</button><button className={tab === 'teams' ? 'selected' : ''} aria-pressed={tab === 'teams'} onClick={() => setTab('teams')}>Constructors</button></div>
          ...
          <div className="championship-guide"><CircleHelp size={18} /><div><strong>THREE HEATS. ONE GRAND PRIX.</strong><p>Race the same track three times. Every heat scores points. Both teammates contribute to the constructors' title.</p><button className="text-button" onClick={() => setDialog('rules')}>The championship format <ArrowRight size={13} /></button></div></div>
        </aside>
      </div>
      <footer className="app-footer"><span>POWERED BY GRAVITY. FUELED BY COMPETITION.</span><button className="text-button" onClick={() => setDialog('lab')}><FlaskConical size={13} />Test the engine</button></footer>
    </main>
    {dialog === 'rules' && <RulesDialog onClose={() => setDialog(null)} />}{dialog === 'lab' && <PhysicsLab onClose={() => setDialog(null)} />}
  </div>;
}