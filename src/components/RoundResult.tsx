...
export default function RoundResult({ result, session, onContinue, onMenu, onNewGame }: RoundResultProps) {
  ...
  // T02: standings tables stay bounded at 100 racers — with an explicit "of N" note
  // so nothing is silently dropped (full standings stay in the saved record).
  const fieldRows = resultRows(resultField(result), PLAYER_ID, RESULTS_MAX_ROWS);
  const rows = fieldRows.rows;
  const cupRows = table.length > RESULTS_MAX_ROWS
    ? (() => {
        const sliced = table.slice(0, RESULTS_MAX_ROWS - 1);
        const player = table.find((row) => row.id === PLAYER_ID);
        if (player && !sliced.some((row) => row.id === PLAYER_ID)) sliced.push(player);
        return sliced;
      })()
    : table;
  const playerCup = table.findIndex((row) => row.id === PLAYER_ID) + 1;
