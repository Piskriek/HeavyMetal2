...
export default function RoundResult({ result, session, onContinue, onMenu, onNewGame }: RoundResultProps) {
  ...
  // P11: the split column and the climber, from the whole field (not just the rows shown).
  const hasSplits = resultField(result).some((row) => row.splitPosition !== undefined);
  const climber = biggestClimber(resultField(result));
