    ...
    <Modal title="The Tinker's Settings" eyebrow="A FEW ADJUSTMENTS. NOTHING EXPLOSIVE." onClose={onClose} className="fantasy-dialog settings-dialog" wide backdrop="settings">
      ...
      <div className="settings-page" role="tabpanel">
        {tab === 'display' && <>
          ...
          <div className="fantasy-setting-row"><div><h3><label htmlFor="menu-ball-camera">Camera</label></h3><p>Cockpit is the driver's own view through the armoured window. Chase sits right above and slightly back from your ball. Fixed course holds the classic wide broadcast view.</p></div><select id="menu-ball-camera" value={options.cameraMode} onChange={(event) => change('cameraMode', event.target.value as GameOptions['cameraMode'])}><option value="first_person">Cockpit (first person)</option><option value="follow_ball">Chase the ball</option><option value="fixed">Fixed course</option></select></div>
