...
export interface CourseDefinition {
  ...
  description: string;
  stadium: string;
  /** Stage 1 alpine downhill profile (TICKET-08 splices Section 2 onto it). */
  profile: readonly (readonly [number, number])[];
  /** Stage 1 sector names; Section 2's are appended from `stage-two.ts`. */
...
export const TRACKS: Record<CourseId, CourseDefinition> = {
  ridge: {
    ...
    sectors: ['THE LAUNCH RIDGE', 'COPPERWOOD DESCENT', 'THE SCREAMING SLOPE', 'SHEPHERD\'S REST', 'DEADWEIGHT DROP', 'THE LOWER SWITCHBACK'],
  ...
  boomtown: {
    ...
    sectors: ['THE QUARRY GATE', 'COPPER CANYON', 'POWDERKEG STRAIGHT', 'THE BIG DROP', 'SMOKESTACK SHELF', 'DYNAMITE DESCENT'],
  ...
  sheep: {
    ...
    sectors: ['THE HILLTOP PADDOCK', 'WOOLWIND MEADOW', 'RAMBLER\'S RUN', 'THE GRAZING SHELF', 'SHEEP LEAP', 'LOWER PASTURES'],
