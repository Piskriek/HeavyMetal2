...
test('Browser: a complete long heat pays winnings and the saved season advances correctly', { timeout: 240000 }, async () => {
  ...
  try {
    ...
    const saved = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-season-v1')!));
    assert.equal(saved.results[0].length, 1, 'Finished heat was not saved immediately.');
    const paid = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    ...
    assert.ok(await page.getByRole('button', { name: 'START HEAT 2', exact: true }).isVisible());
    assert.equal((await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-season-v1')!))).seed, saved.seed);
    assert.equal((await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!))).credits, paid.credits, 'Returning to the paddock or reloading duplicated the payout.');
    assert.deepEqual(errors, []);
  } finally { await context.close(); }
});
...
test('Browser: shop purchases persist, number keys spend only selected items, and quitting pays nothing', { timeout: 60000 }, async () => {
  ...
  try {
    ...
    let account = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    ...
    account = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    const initialBoost = account.inventory.rocket;
    await page.keyboard.press('1');
    const afterBoost = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    ...
    const beforeJump = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    await page.keyboard.press('2');
    const afterJump = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    ...
    const beforeAero = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    await page.keyboard.press('6');
    const afterAero = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    assert.equal(afterAero.inventory.aero, beforeAero.inventory.aero - 1);
    await page.clock.runFor(100);
    const used = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
    ...
    const exited = await page.evaluate(() => JSON.parse(localStorage.getItem('mrr-account-v1')!));
