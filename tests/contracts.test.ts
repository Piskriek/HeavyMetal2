...
test('commands: the gate decides what is legal, and refusals are typed', () => {
  ...
  assert.equal(validateCommand({ type: 'steer', direction: 1 }, gateState({ status: 'flying', phase: 'racing' })).ok, true, 'steering while racing');
  assert.equal(validateCommand({ type: 'steer', direction: -1 }, gateState({ status: 'paused', phase: 'racing' })).ok, true, 'steering while paused');
  assert.equal(validateCommand({ type: 'steer', direction: 1 }, gateState({ status: 'flying', phase: 'racing', inputEnabled: false })).ok, false);
