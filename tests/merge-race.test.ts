...
function runMerge(seed: number, course: CourseId = 'ridge', skipHeldGhost = true): RaceResult {
  ...
  for (; tick < budget; tick++) {
    ...
    // 4. The pool: occupancy from the previous release's own ride, then the ordered release.
    ...
    for (const racerId of released) {
      ...
      // Pinned to the ring's own lane, or the lane-filtered loop would let them run straight past it.
    ...
    for (let i = 0; i < racers.length; i++) {
      ...
      mergeStepMax = Math.max(mergeStepMax, moved);
      // A ring ride travels an arc, and an arc loses ground at the top of the ring; a fall can also
      // lose ground. Everything else only ever gains it.
      const arcing = before[i].loop != null || racers[i].loopRide !== null || racers[i].falling;
      if (!arcing) {
        mergeStepMin = Math.min(mergeStepMin, moved);
        if (moved < -1e-9) backwardOnFlat += 1;
      }
    }

    // 6. Ghost tails, and the two events worth recording: engaging the ring and leaving it.
    for (let i = 0; i < racers.length; i++) {
      const racer = racers[i];
      if (racer.mergeGhost && racer.loopExitTime > -100 && runTime >= racer.loopExitTime + MERGE_GHOST_TAIL_S) {
        racer.mergeGhost = false;
      }
    ...
    if (mergeFinishedTick >= 0 && racers.every((racer) => !racer.mergeGhost)) break;
  }

  let minExitSpacing = Number.POSITIVE_INFINITY;
  const byX = [...exited].sort((a, b) => b.racer.x - a.racer.x);
  for (let index = 1; index < byX.length; index++) {
    minExitSpacing = Math.min(minExitSpacing, byX[index - 1].racer.x - byX[index].racer.x);
  }
...
test('no contacts while held or ghost', () => {
  ...
  // And the instrument is not blind: with the filter off, the same runs *do* bump held riders. That
  // is the whole reason the filter exists — a queued rider shoved sideways would lose their place.
  const unfiltered = runMerge(DEFAULT_PUSH_SEED, 'ridge', false);
  assert.ok(unfiltered.bumpsOnHeldOrGhost > 0,
    'the no-bump check is vacuous unless turning the filter off actually produces bumps');
});
...
test('the engine drives the pool the way this harness does', () => {
  ...
  assert.match(source, /racer\.targetLane = closestLane\(obstacleZ\(this\.mergeGateFor\(\)\.loop\)\);/,
    'a release must aim the rider at the loop\'s own lane, or the lane-filtered ring will not take them');
...
test('pool retires after the last release', () => {
  ...
  assert.ok(run.exitTick > 0 && run.exitTick < run.ticks, 'the ring empties before the run stops');
});
